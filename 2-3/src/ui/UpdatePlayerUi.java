...
public class UpdatePlayerUi extends AbstractUi {
	...
	protected Player getPlayer() {
		...
		if (StringUtils.isEmpty(id)) { //문자열이 입력되어 있는지?
			return null;
			
		} else if (UiUtils.isNumeric(id, playerId)) { // 숫자인지?
			//ID로 선수를 검색
			Player player = this.playerDao.getPlayer(Integer.valueOf(id));
			
			if (player == null) { //해당하는 선수가 존재하지 않는다.
				System.out.printf("입력하신 선수ID가 '%s'인 선수는 존재하지 않습니다.%n", id);
				return getPlayer();
			}
